...
| No.  | Date     | Name   | Function                                                         | Done                                                         |
| ---- | -------- | ------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
|     1|2024/01/05|        | 가격표 크롭 | 직접 찍은 이미지에서 가격표만 크롭하기 구현 - Roboflow API 사용 |
|     2|2024/01/05|        | 가격표 크롭 | 가격표만 추출하는 모델 추가적으로 서치 |
|     3|2024/01/05|        | 상품명 추출 | 가격표 내 텍스트 전체 불러오기 |
|     4|2024/01/08|        | 상품 정보 크롤링 | 쇼핑몰 사이트 크롤링해서 상품 정보 가져오기 |
|     5|2024/01/08|        | 가격표 크롭 | Roboflow API로 크롭되지 않은 이미지는 contour로 가격표 인식해서 가져오도록 구현 |
|     6|2024/01/09|        | 가격표 크롭 및 OCR | 가격표만 정교하게 크롭한 후 OCR API로 이미지 내 텍스트만 추출 |
|     7|2024/01/15|        | 상품명 추출 | 텍스트 내에서 상품명과 가격 추출 |
|     8|2024/01/16|        | 웹 구현 (프론트) | 이미지 업로드 및 업로드 된 이미지 불러오기 |
|     9|2024/01/17|        | 웹 구현 (백)    | 이미지에서 크롭 -> OCR -> NER 진행 후 <상품명>과 <가격>, <크롭된 이미지> 보여주기 구현 |
|    10|2024/01/18|        | 상품명 추출 | ko_core_news_lg 모델, ner + attributer_ruler 학습한 모델<br />**- 추가적인 모델링 작업으로 모델 성능 높이는 중** |
|    11|2024/01/19|        | 상품명 추출 | 상품명만 인식하는 모델, 가격만 인식하는 모델 생성 |
|    12|2024/01/19|        | 웹 구현 | React, Django 연결 |
...
|     4|        | 가격을 태깅할 때 문제<br />- 가격이 텍스트의 앞쪽에 위치한 경우 가격 태그로 인식하지 못하는 문제<br />-가격 태그가 2개 이상 추출되는 경우가 발생
