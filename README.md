...
|김정연| 모델이 우리가 원하는 기능을 갖추도록 학습시키기 위해 데이터 전처리하는데 시간을 많이 투자해보았습니다. 전처리를 통해 질 좋은 데이터를 생성해 내는 것이 모델학습에 중요하다는 것과 전처리하는 시간이 전혀 아까운 시간이 아님을 다시 한번 느꼈습니다. 또한 웹페이지 구성을 같이 진행을 해보며 Django를 활용한 웹페이지 구축에 대한 이해도를 증진시킬 수 있었습니다 |
...
| ---- | ------ | ------------------------------------------------------------ | -------- | ------------------------------------------------------------ |
|     1|        | 직접 찍은 이미지에서 가격표만 크롭하는데 어려움<br />- 가격표만 인식하는 모델들은 몇 개 있지만 대부분이 외국어 데이터 | 해결 완료 | 직접 찍은 데이터를 기준으로 데이터 어노테이션 및 어그멘테이션으로 데이터 확보 환료 |
